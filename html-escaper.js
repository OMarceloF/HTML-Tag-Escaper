...
function espaceHtmlSpecialCharacters(text){
...
function escapeHtmlFile(inputFilePath, outputFilePath){
  try {
    ...
    const escapedContent = espaceHtmlSpecialCharacters(fileContent)
...
async function userInteraction(){
  ...
  const answer = await askFilePath(`Informe o caminho do arqquivo de saída (padrão: ${defaultName}):`)
...
function run(){
  if (process.argv.length >= 4){
    ...
    console.log("------------------\r")
    console.log("Argumentos não informados! Por favor, informe os caminhos para que o programa seja executado!")
    userInteraction()
  }
}